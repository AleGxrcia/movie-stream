import { FiChevronLeft, FiFilm, FiHome, FiLogOut, FiMessageSquare, FiUsers } from "react-icons/fi";
        ...
        <aside className="w-64 h-screen bg-[#101828] text-white flex flex-col">
            ...
            <div className="px-6 py-4 border-b border-gray-700">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-xs text-blue-500 capitalize">{user?.roles.join(', ')}</p>
...
const SidebarItem = ({ icon, label, to }: { icon: React.ReactNode; label: string, to: string }) => (
    <NavLink
        ...
        className={({ isActive }) =>
            `w-full flex items-center gap-3 px-2 py-2 rounded transition ${isActive ? "bg-blue-500/20 text-blue-300" : "hover:bg-white/10"
            }`
        }
    >
        {icon}
